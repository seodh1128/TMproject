...
public class TempActivity extends Activity{
    ...
    public void onCreate(Bundle saveInstanceState){
        ...
        Toast.makeText (this, "온도센서 상태 확인 페이지입니다",Toast.LENGTH_LONG).show();
    ...
    public void temp() {
        ...
        dashboardCall.enqueue(new Callback<List<Dashboard>>() {
            ...
            public void onResponse(Call<List<Dashboard>> call, Response<List<Dashboard>> response) {
                ...
                Dashboard d = result.get(2);    //result.get(0)이 쿼리문에 지정한 값들의 한줄을 읽어오는거 나머지 센서3개는 get1,2,3 찾아서 해주묜 됌
  ...
  /*  public void temp2(int deviceCode,int sensorCode){
        Call<Dashboard> dashboardCalldetail = dashboardApi.getDetailList(deviceCode,sensorCode);

        dashboardCalldetail.enqueue(new Callback<Dashboard>() {
            @Override
            public void onResponse(Call<Dashboard> call, Response<Dashboard> response) {
                Dashboard result = response.body();
                Gson gson = new Gson();
                Log.d("9", "onResponse: "+result.toString());


                name.setText(result.getDeviceName());
                value.setText(Double.toString(result.getSensorValue()));
                condition.setText(Integer.toString(result.getAutoMode()));
                state.setText(Integer.toString(result.getDeviceState()));
                date.setText(newDate(result.getRegDate()));
