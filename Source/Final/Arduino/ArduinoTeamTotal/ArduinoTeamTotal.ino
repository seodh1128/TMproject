...
const long luxInterval = 5000;
const long luxBright = 700;
...
class ButtonCheck
{
  ...
  void Update()
  {
    ...
    if(buttonState == LOW)
    {
      ...
      if(btnCurrentMillis - btnPreviousMillis >= secuInterval)
      {
...
void security(){
  ...
  //  Security관련 동작 실행 - (버튼 : 근접 센서/감지기)
    ...
    if(secuCurrent - secuPre  >= secuInterval){
...
void lux(){
  ...
  if(luxAMState==1){
      if (photoResistorValue >= luxBright ) {
...
void th(){
    ...
    if(cur_time - pre_time >= ehpInterval){
      pre_time = cur_time;
